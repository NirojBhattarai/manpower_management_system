...
import { ExperienceListResponse } from "../interface/experience-interface";
...
const useExperienceList = () => {
  ...
  const { data, isLoading } = useGetDataQuery<{
    ...
    url: endpoints.candidates.experience.list.replace(":id", jobSeekerId ?? ""),
    params: {
      page: pagination.pageIndex + 1,
      perPage: pagination.pageSize,
      search: debouncedSearch,
    },
    tag: apiTags.candidates.experience.list,
  });

  return {
    experienceListResponse: data,
    isLoading,
    rowSelection,
    setRowSelection,
  };
